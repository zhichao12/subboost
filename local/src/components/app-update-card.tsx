...
export function AppUpdateCard({ disabled = false }: { disabled?: boolean }) {
  ...
  const refresh = React.useCallback(async () => {
    ...
    setRelease(normalizeReleaseStatus((await releaseResponse.json()) as ReleaseStatus));
    setUpdate(normalizeAppUpdateStatus((await updateResponse.json()) as AppUpdateStatus));
  }, []);
    ...
    <Card className="lg:col-span-2">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" className="gap-2" onClick={() => void refresh().catch(() => setError("更新状态加载失败，请刷新重试"))} disabled={loading || applying || disabled}>
